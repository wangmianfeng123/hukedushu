...
- (void)createUI {
    ...
    if (HKVideoType_JobPath ==  [self.detailModel.video_type integerValue]) {
        ...
        [self.buyVipTipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(self.jobPathVipBtn.mas_top).offset(-27);
        }];
...
- (void)makeConstraints {
    ...
    [self.buyVipTipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.buyVipBtn.mas_top).offset(-25);
    ...
    self.collectBtn.hidden = isCollect ?YES :NO;
    if (_playerShareBtn.hidden == NO) {
...
- (UIButton*)collectBtn {
    ...
    if (!_collectBtn) {
        ...
        [_collectBtn setTitleColor:[COLOR_ffffff colorWithAlphaComponent:0.8] forState:UIControlStateHighlighted];
...
//        [MobClick event:detailpage_sharelimit_buy];
//        [MobClick event:UM_RECORD_LIMIT_BUTTON];
        
        if (self.permissionVideoModel.is_video_share_over_num) {
            [HKALIYunLogManage sharedInstance].button_id = @"10";
            [MobClick event: detailpage_sharelimit_buy];
            
        }else{
            [MobClick event: UM_RECORD_LIMIT_BUTTON];
            [HKALIYunLogManage sharedInstance].button_id = @"9";
        }
    }
}
...
//    }
    [MobClick event: @"C07002"];//观看受限弹窗点击活动
