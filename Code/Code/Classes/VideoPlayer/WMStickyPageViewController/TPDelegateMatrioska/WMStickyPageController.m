...
- (void)scrollView:(WMMagicScrollView *)scrollView scrollViewDidScrollWithSubview:(UIScrollView *)subview {
    if ([self.delegate conformsToProtocol:@protocol(WMStickyPageControllerDelegate)]) {
        id<WMStickyPageControllerDelegate> delegate = (id<WMStickyPageControllerDelegate>)self.delegate;
        if ([delegate respondsToSelector:@selector(pageController:scrollViewDidScrollWithSubview:)]) {
            return [delegate pageController:self scrollViewDidScrollWithSubview:subview];
        }
    }
}
...
- (void)setContentViewContentOffset:(CGFloat)Y {
    ...
    [self.contentView setContentOffset:CGPointMake(0 ,Y)];
}
