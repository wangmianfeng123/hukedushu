...
//  Code
//
//  Created by Ivan li on 2019/6/4.22333
...
@implementation HKJobPathHotCell
...
- (void)makeConstraints {
    ...
    [self.titleLB mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //make.left.equalTo(self.bgIV).offset(PADDING_25);
    ...
    [self.imageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //make.right.equalTo(self.bgIV).offset(-PADDING_25);
...
- (void)setModel:(HKJobPathModel *)model {
    ...
    self.descrLB.attributedText = [NSMutableAttributedString changeLineSpaceWithTotalString:isEmpty(model.desc)?@" ": model.desc LineSpace:5];
    
    //NSString *text = [NSString stringWithFormat:@"%@人已学   %ld个章节   %ld节课",model.study_number,model.chapter_number,model.course_number];
    ...
    if (@available(iOS 13.0, *)) {
        ...
        self.bgIV.image = [UIImage resizableImageWithName:(DMUserInterfaceStyleDark == mode) ? @"bg_careerlist_v2_13_dark" :@"bg_careerlist_v2_13"];
    }else{
        self.bgIV.image = [UIImage imageNamed:@"bg_careerlist_v2_13"];
    }
...
- (void)dmTraitCollectionDidChange:(DMTraitCollection *)previousTraitCollection {
    //[UIImage hkdm_imageWithNameLight:@"bg_careerlist_v2_13" darkImageName:@"bg_careerlist_v2_13_dark"]
    if (@available(iOS 13.0, *)) {
        DMUserInterfaceStyle mode = DMTraitCollection.currentTraitCollection.userInterfaceStyle;
        //self.bgIV.hidden = (DMUserInterfaceStyleDark == mode) ? YES :NO;
