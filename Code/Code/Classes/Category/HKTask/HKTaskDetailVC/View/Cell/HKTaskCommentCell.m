...
+ (instancetype)initCellWithTableView:(UITableView *)tableView  row:(NSInteger)row {
    ...
    if (!cell) {
        cell = [[HKTaskCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"HKTaskCommentCell" row:row];
    }
    cell.row = row;
    return cell;
}
...
- (void)setCommentM:(HKTaskCommentModel *)commentM {
    _commentM = commentM;
    //self.attrLabel.text = [NSString stringWithFormat:@"%@: %@", commentM.username, commentM.content];
    ...
    if (self.row>2) {
        ...
        if (!isEmpty(commentM.username)) {
            ...
            paragraphStyle.lineBreakMode = NSLineBreakByTruncatingTail;//末尾.....
