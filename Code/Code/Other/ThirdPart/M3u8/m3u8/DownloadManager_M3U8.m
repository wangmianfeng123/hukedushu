...
//    [[HKDownloadManager shareInstance] HKM3U8ParseSave:modelChange];
//    
//    return;
    
    NSError *error = nil;
    self.downloading_m3u8SegmentList = [self.analyser analyseVideoUrl:downloadModel.url error:&error];
    HKM3U8Parse *parse = [[HKM3U8Parse alloc] init];
    
    //HKDownloadModel *list = [parse analyseVideoUrl:downloadModel.url error:&error videoID:downloadModel.videoId videoType:downloadModel.videoType];
    //NSLog(@"%@", list);
    if (error)
    {
