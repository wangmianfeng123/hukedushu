...
- (void)makeConstraints {
    ...
    [_iconImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.contentView.mas_top).offset(PADDING_20);
...
- (UILabel*)titleLabel {
    ...
    if (!_titleLabel) {
        ...
        // [UIFont systemFontOfSize:IS_IPHONE6PLUS ? 17:15];
    ...
    if (!_honorLabel) {
        _honorLabel  = [UILabel labelWithTitle:CGRectZero title:nil//@"虎课明星讲师"
                                    titleColor:COLOR_A8ABBE
                                     titleFont:nil
                                 titleAligment:NSTextAlignmentLeft];
        _honorLabel.font = HK_FONT_SYSTEM(IS_IPHONE6PLUS ? 15:13);
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    CGFloat totalHeight = PADDING_40;
    CGSize imageViewSizeTemp = IS_IPHONE6PLUS ? CGSizeMake(PADDING_30*2, PADDING_30*2) : CGSizeMake(PADDING_25*2, PADDING_25*2);
    totalHeight += imageViewSizeTemp.height;
