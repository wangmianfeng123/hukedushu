...
/**
 底部按钮点击事件，拓展scrollView
 
 @param btn 按钮
 @param status 网络状态
 */
- (void)tb_emptyButtonClick:(UIButton *)btn network:(TBNetworkStatus)status scrollView:(UIScrollView *)scrollView;
...
@property (nonatomic, weak)id<TBSrollViewEmptyDelegate> tb_EmptyDelegate; // 代理

/**
 自定义的emptyView
 
 用于特殊情形 自定义 emptyView
