//
//  HomeRecomdCell.m
//  Code
//
//  Created by Ivan li on 2017/10/12.
//  Copyright © 2017年 pg. All rights reserved.
//

#import "HomeMyFollowCell.h"

@implementation HomeMyFollowCell


- (instancetype)initWithFrame:(CGRect)frame {
    
    self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40)];
    if (self) {
        [self createUI];
    }
    return self;
}

- (void)createUI {
    
    [self addSubview:self.titleLabel];
    [self addSubview:self.grayLabel];
    [self addSubview:self.tagLabel];
    [self addSubview:self.changeBtn];
    [self addSubview:self.bottomSeparator];
    
    [self makeConstraints];
    self.backgroundColor = COLOR_F8F9FA_333D48;
    self.titleLabel.textColor = COLOR_27323F_EFEFF6;
    self.bottomSeparator.backgroundColor = COLOR_F8F9FA_333D48;
    self.grayLabel.backgroundColor = COLOR_F8F9FA_333D48;
}

- (void)makeConstraints {
    [_grayLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.right.equalTo(self);
        make.height.mas_equalTo(8);
    }];
    
    [_tagLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self);
        make.height.equalTo(@14);
        make.bottom.mas_equalTo(self).mas_offset(-4);
        make.width.mas_equalTo(2);
    }];
    
    [_titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self).mas_offset(5);
        make.left.equalTo(self).offset(PADDING_15);
    }];
    
    [_changeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.height.equalTo(self.titleLabel);
        make.right.equalTo(self.mas_right).offset(-PADDING_15 - 2);
        //make.width.mas_equalTo(PADDING_20*4);
    }];
    
    // 底部1pt 的分割线
    [self.bottomSeparator mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.titleLabel);
        make.right.bottom.mas_equalTo(self);
        make.height.mas_equalTo(1);
    }];
}


- (UIView *)bottomSeparator {
    if (_bottomSeparator == nil) {
        _bottomSeparator = [[UIView alloc] init];
        _bottomSeparator.backgroundColor = HKColorFromHex(0xF8F9FA, 1.0);
    }
    return _bottomSeparator;
}

- (UILabel*)titleLabel {
    
    if (!_titleLabel) {
        _titleLabel  = [[UILabel alloc] init];
        [_titleLabel setTextColor:HKColorFromHex(0x27323F, 1.0)];
        
        _titleLabel.textAlignment = NSTextAlignmentLeft;
        _titleLabel.font = [UIFont systemFontOfSize:IS_IPHONE6PLUS ? 18: 17 weight:UIFontWeightMedium];
        _titleLabel.text = @"我的关注";
    }
    return _titleLabel;
}

- (UILabel*)grayLabel {
    if (!_grayLabel) {
        _grayLabel  = [UILabel new];
        _grayLabel.backgroundColor = HKColorFromHex(0xF8F9FA, 1.0);
    }
    return _grayLabel;
}


- (UILabel*)tagLabel {
    if (!_tagLabel) {
        _tagLabel  = [UILabel new];
        _tagLabel.hidden = YES;
        _tagLabel.backgroundColor = COLOR_333333;
    }
    return _tagLabel;
}



- (UIButton*)changeBtn {
    
    if (!_changeBtn) {
        _changeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_changeBtn setImage:imageName(@"home_my_follow_more") forState:UIControlStateNormal];
        [_changeBtn setTitle:@"更多" forState:UIControlStateNormal];
        [_changeBtn setTitleColor:HKColorFromHex(0xA8ABBE, 1.0) forState:UIControlStateNormal];
        [_changeBtn.titleLabel setFont:[UIFont systemFontOfSize:IS_IPHONE6PLUS ? 14:13]];
        _changeBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
        [_changeBtn addTarget:self action:@selector(changeVideoAction:) forControlEvents:UIControlEventTouchUpInside];
        _changeBtn.titleLabel.lineBreakMode = NSLineBreakByClipping;
        //_changeBtn.width = PADDING_20*6;
        [_changeBtn layoutButtonWithEdgeInsetsStyle:HKButtonEdgeInsetsStyleRight imageTitleSpace:5];
        
//        [_changeBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, -_changeBtn.imageView.image.size.width, 0, _changeBtn.imageView.image.size.width)];
//
//        CGFloat imageOffset = IS_IPHONE6PLUS? 63 : 58;
//        [_changeBtn setImageEdgeInsets:UIEdgeInsetsMake(0, imageOffset, 0, -imageOffset)];
        
        _changeBtn.hidden = YES;
    }
    return _changeBtn;
}



- (void)changeVideoAction:(id)sender {
    !self.self.moreFollowBlock? : self.moreFollowBlock();
}

- (void)setFollow_list_type:(NSInteger)follow_list_type {
    
    //1-我的关注列表 2-推荐讲师列表;
    if (follow_list_type == 1) {
        self.titleLabel.text = @"我的关注";
    }else {
        self.titleLabel.text = @"安利墙";
    }
    // 显示隐藏更多按钮
    //self.changeBtn.hidden = follow_list_type == 2;
    self.changeBtn.hidden = NO;
}








@end
