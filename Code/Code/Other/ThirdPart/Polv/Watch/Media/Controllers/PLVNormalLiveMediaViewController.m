...
- (void)viewDidLoad {
    ...
    self.player.pauseInBackground = YES; // 默认回后台暂停
    ((PLVLivePlayerController *)self.player).cameraClosed = NO;
    [(PLVLivePlayerController *)self.player setChaseFrame:self.chaseFrame];
    WeakSelf
    self.skinView.changeDeviceOrientationBlock = ^(UIDeviceOrientation orientation) {
        [weakSelf manChangedeviceOrientationDidChange:orientation];
    };
}
