...
- (void)unflodAction:(UIButton *)sender {
    ...
    if(self.needChangToFrame) {
        self.needChangToFrame(self.frame,self.isUnfolded);
    }
    self.unfoldBtn.hidden = self.isUnfolded ;
}
...
- (void) setUnflodBtnWithStatus:(BOOL)isUnfolded {
    ...
    NSString *title = self.unflodBtnModel.title ? self.unflodBtnModel.title : [NSString stringWithFormat:@"全部%ld个",self.tags.count];
    UIImage *image = [UIImage imageNamed:@"ic_more_tag_2_34"];
    
    self.displayLines = self.minDisplayLines;
    
    if(isUnfolded) {
        title = self.unflodBtnModel.selectedTitle ? self.unflodBtnModel.selectedTitle :  @"收起";
        image = [UIImage imageNamed:@"ic_more_tag_2_34"];
    ...
    //设置标题颜色
    [self.unfoldBtn setTitleColor:self.unflodBtnModel.normalTitleColor ? self.unflodBtnModel.normalTitleColor : COLOR_7B8196_A8ABBE forState:UIControlStateNormal];
    ...
    [self.unfoldBtn setBackgroundColor:[UIColor clearColor]];
    ...
    bounds.size.width = ceilf(5 + titleWidth + image.size.width);
    bounds.size.height = ceilf(titleHeight / 2 * 3);
    
    UIEdgeInsets titleEdgeInsets = UIEdgeInsetsMake(0, 0, 0, ceilf(image.size.width));
    UIEdgeInsets imageEdgeInsets = UIEdgeInsetsMake(0, ceilf(titleWidth + image.size.width + 5), 0, 0);
