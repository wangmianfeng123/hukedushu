...
@interface HKVIPOneYearVC ()<UITableViewDelegate,UITableViewDataSource,PayResultDelegate, HKOtherVipMidCellDelegate>
...
@property(nonatomic,strong)HKMapModel *adModel;
...
@property (nonatomic, copy) NSString * autoRenewalString;//说明文案子串
@property (nonatomic, assign) BOOL isAutoBuy;//是否自动续费
@property (nonatomic, assign) BOOL isAgree;//是否同意协议
@property (nonatomic, strong)HKBuyVipModel *autoRenewalModel; // 当前选中的model
@property (nonatomic, strong) HKVIPProtocolView * protocolView;

@end

@implementation HKVIPOneYearVC
...
- (NSMutableArray<HKVipPrivilegeModel *> *)vipPrivilegeDataSource {
    if (_vipPrivilegeDataSource == nil) {
        ...
        model7.header = @"ic_mycareer_v2_13";
        model7.name = @"职业路径无限学";
        model7.des = @"不断更新中";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.isAgree = YES;
    self.autoRenewalString = @"1、付款：确认购买后，即从您的苹果iTunes账户扣费\n2、续费：在您订阅周期到期前24小时，苹果会自动从iTunes账户扣费，成功后服务有效期将在48小时内延长一个订阅周期\n3、如需取消自动续费，请在订阅到期前至少24小时，手动在Apple ID账户设置中关闭自动续费，关闭后不再扣费\n4、自动续费活动不与其他优惠活动同享，不支持升级vip，敬请谅解。";
...
- (UITableView*)tableView {
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([HKOneYearProtocolCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([HKOneYearProtocolCell class])];
...
- (void)makeConstraints {
    ...
    self.protocolView.autoBuyBtnBlock = ^{
        @strongify(self);
        NSString * webUrl;
        if (self.autoRenewalModel.agreements.count > 1) {
            NSDictionary * dic = self.autoRenewalModel.agreements[1];
            webUrl = dic[@"url"];
        }else{
            webUrl = [NSString stringWithFormat:@"%@%@",[CommonFunction HK_BaseUrl_NO_V5],SITE_VIP_AGREEMENT];
        }
    ...
    self.protocolView.agreementBtnBlock = ^{
        @strongify(self);
        NSString * webUrl;
        if (self.autoRenewalModel.agreements.count > 0) {
            NSDictionary * dic = self.autoRenewalModel.agreements[0];
            webUrl = dic[@"url"];
        }else{
            webUrl = [NSString stringWithFormat:@"%@%@",[CommonFunction HK_BaseUrl_NO_V5],SITE_VIP_AGREEMENT];
        }
...
- (UIButton*)buyBtn {
    if (_buyBtn == nil) {
        ...
        UIColor *color = [UIColor colorWithHexString:@"#ff8d00"];
        UIColor *color1 = [UIColor colorWithHexString:@"#ffa000"];
        UIColor *color2 = [UIColor colorWithHexString:@"#ffb400"];
...
//
//        if (self.autoRenewalModel.canAutoRenewal && self.isAutoBuy) {
//            self.autoRenewalModel.canAutoRenewal = 1;
//            [apPay buyProductionModel:self.autoRenewalModel];
//        }else{
//            [apPay buyProductionModel:self.selectedModel];
//        }
//    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat aotoBuyHeight = [self.autoRenewalString boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 2 * 15.0, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size.height + 40;
    ...
    switch (indexPath.section) {
        ...
        case 1:// 自动续费
        {
            if (self.autoRenewalModel.canAutoRenewal) {
                height = 82;
            }else{
                height = 0.1;
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (self.isAutoBuy) {
            cell.model = self.autoRenewalModel? self.autoRenewalModel : self.vipDataSource.firstObject;
            
        }else{
            cell.model = self.selectedModel? self.selectedModel : self.vipDataSource.firstObject;
        }
        cell.isAutoBuy = self.isAutoBuy;
        ...
        return cell;
    } else if (indexPath.section == 3) { // 即将到来的分类
        ...
        HKVIPPrivilegeStringCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([HKVIPPrivilegeStringCell class])];
        cell.txt = self.autoRenewalString;
        return cell;
    }
    return nil;
}
...
-(void)setIsAutoBuy:(BOOL)isAutoBuy{
    ...
    [self.buyBtn setTitle:isAutoBuy ? self.autoRenewalModel.buy_button_title :@"立即开通" forState:UIControlStateNormal];
    self.protocolView.isOpenAutoBuy = self.isAutoBuy;
...
- (void) getVIPList{
    ...
    [HKHttpTool POST:@"/vip/vip-list" parameters:nil success:^(id responseObject) {
        ...
        if (HKReponseOK) {
            HKBuyVipModel *model = [HKBuyVipModel mj_objectWithKeyValues:responseObject[@"data"][@"all_vip_data"]];            
            model.button_type = self.button_type;
            
            
            self.autoRenewalModel = [HKBuyVipModel mj_objectWithKeyValues:responseObject[@"data"][@"allVipAutoRenewalInfo"]];
            self.isAutoBuy = self.autoRenewalModel.canAutoRenewal;
            self.protocolView.isOpenAutoBuy = self.isAutoBuy;
            // 赋值广告位信息
            HKMapModel *adMapModel = nil;
            if ([responseObject[@"data"][@"advertises"] isKindOfClass:[NSDictionary class]] && [responseObject[@"data"][@"advertises"][@"all_vip_data"] isKindOfClass:[NSDictionary class]]) {
                adMapModel = [HKMapModel mj_objectWithKeyValues:responseObject[@"data"][@"advertises"][@"all_vip_data"]];
                model.ad = adMapModel;
                self.autoRenewalModel.ad = adMapModel;
            }
