...
static const BOOL tb_isShowDetailLB = NO; // 显示详情
...
- (NSInteger)tb_totalDataCount
{
    ...
    if ([self isKindOfClass:[UITableView class]]) {
        ...
        //        NSLog(@"xixixixi    %ld", collectionView.numberOfSections);
        //        if (totalCount == 0) {
...
-(void)btnClick:(UIButton *)btn {
    ...
    if ([self.tb_EmptyDelegate respondsToSelector:@selector(tb_emptyButtonClick:network:)]) {
    ...
    } else if ([self.tb_EmptyDelegate respondsToSelector:@selector(tb_emptyButtonClick:network:scrollView:)]) {
        [self.tb_EmptyDelegate tb_emptyButtonClick:btn network:[self networkdStatus] scrollView:self];
    } else {
        ...
        }else if ([reponder respondsToSelector:@selector(tb_emptyButtonClick:network:scrollView:)]) {
            id tb_EmptyDelegateTemp = reponder;
            [tb_EmptyDelegateTemp tb_emptyButtonClick:btn network:[self networkdStatus] scrollView:self];
        }
    }
}
