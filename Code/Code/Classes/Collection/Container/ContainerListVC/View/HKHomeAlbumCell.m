...
@implementation HKHomeAlbumCell
...
- (void)hkDarkModel {
    ...
    self.nameLabel.textColor = COLOR_A8ABBE_7B8196;
...
- (void)makeConstraints {
    ...
    [_coverImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(285/2, 184/2));
    }];

    [_titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.coverImageView).mas_offset(2);
    ...
    [_collectLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(_coverImageView.mas_bottom).offset(-2);
        make.left.equalTo(self.titleLabel.mas_left);
    }];
    
    [_couseLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.height.equalTo(_collectLabel);
...
//    _nameLabel.text = model.username;
    ...
    _couseLabel.text = [NSString stringWithFormat:@"%@人收藏",model.collect_num];
    _collectLabel.text = [NSString stringWithFormat:@"%@节课",model.video_num];
}
