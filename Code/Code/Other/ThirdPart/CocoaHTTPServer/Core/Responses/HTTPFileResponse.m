...
- (id)initWithFilePath:(NSString *)fpath forConnection:(HTTPConnection *)parent {
	...
	if((self = [super init]))
	{
        ...
        if ([fpath containsString:@".ts"]) {
        ...
        else if ([fpath containsString:@".m3u8"]){
            connection = parent; // Parents retain children, children do NOT retain parents
        ...
        }else if ([fpath containsString:@"/temp"]){
            ...
            NSString *keyStr = [AES128Helper AES128DecryptText:content key:@"e10adc3949ba59abbe56e057f20f883e"];
            
            fileLength =  [keyStr length];
            //fileLength =  (UInt64)[[fileAttributes objectForKey:NSFileSize] unsignedLongLongValue];
            fileOffset = 0;
            aborted = NO;
        }
...
- (NSData *)readDataOfLength:(NSUInteger)length
{
	...
	else // (result > 0)
	{
        ...
        if ([filePath containsString:@"temp"]){
            ...
            NSData *endData = [keyStr dataUsingEncoding:NSUTF8StringEncoding];
            return endData;
        }
        /*********************************************/
