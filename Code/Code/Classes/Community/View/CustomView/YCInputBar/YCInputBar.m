...
-(YCInputBar*)initBar:(UIView *)mainView sendButtonTitle:(NSString *)title maxTextLength:(NSInteger)length isHideOnBottom:(BOOL)isHide buttonColor:(UIColor*)color
{
    if (self = [super init]) {
        ...
        _viewInputBar.backgroundColor=COLOR_FFFFFF_3D4752;
...
//        _btnReply.frame = CGRectZero;
        ...
        [_btnReply sizeToFit];
        _btnReply.frame = CGRectMake(_viewInputBar.bounds.size.width - _btnReply.frame.size.width - 8,_viewInputBar.bounds.size.height-28-8, _btnReply.bounds.size.width, 28);
        ...
        _txtInput.backgroundColor = COLOR_F8F9FA_333D48;
        //设置该属性，让输入框随父view的尺寸改变而改变
        _txtInput.autoresizingMask = UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
        _txtInput.textColor = COLOR_27323F_EFEFF6;
        _txtInput.font = [UIFont systemFontOfSize:13];
        //边框圆角
        _txtInput.layer.cornerRadius = 14;
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    _btnReply.frame = CGRectMake(_viewInputBar.bounds.size.width - _btnReply.frame.size.width - 8,_viewInputBar.bounds.size.height-28-8, _btnReply.bounds.size.width, 28);
    _txtInput.textColor = COLOR_27323F_EFEFF6;    //替换富文本后将光标移动到记录位置
