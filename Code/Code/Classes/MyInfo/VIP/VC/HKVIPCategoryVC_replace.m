...
@interface HKVIPCategoryVC_replace() <UIScrollViewDelegate, HKVIPOneYearVCDelegate>
/** 标签栏底部的红色指示器 */
...
@implementation HKVIPCategoryVC_replace
...
- (void)setRightBarButtonItem {
    if (APPSTATUS) {
        ...
        _rightBarBtn.titleLabel.font = HK_FONT_SYSTEM(12);
        [_rightBarBtn setTitle:@"恢复权益" forState:UIControlStateNormal];
        [_rightBarBtn setTitle:@"恢复权益" forState:UIControlStateSelected];
        ...
        [_rightBarBtn addTarget:self action:@selector(rightBarButtonIteAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)rightBarButtonIteAction {
    [[YHIAPpay instance]restoreRefreshRequest];
}
...
- (void)setupTitlesView
{
    ...
    for (NSInteger i = 0; i < self.childViewControllers.count; i++) {
        ...
        [button setTitleColor:COLOR_7B8196_A8ABBE forState:UIControlStateNormal];
        ...
        [attrNormal addAttribute:NSForegroundColorAttributeName value:COLOR_7B8196_A8ABBE range:NSMakeRange(0, vc.title.length)];
...
{
    if (button.tag == 1) {
        [MobClick event:UM_RECORD_VIP_ALL_TAB];
    }else if (button.tag == 2){
        [MobClick event:UM_RECORD_VIP_ZHONGSHENTAB];
    }
    // 修改按钮状态
