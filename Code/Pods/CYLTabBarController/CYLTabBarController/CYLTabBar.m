...
- (void)layoutSubviews {
    ...
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:NSClassFromString(@"_UIBarBackground")]) {
            ...
            shadowLine.frame = CGRectMake(0, 0, view.frame.size.width, 0.5);
            [view addSubview:shadowLine];
            shadowLine.backgroundColor = [UIColor colorWithWhite:0 alpha:0.1];
            break;
        }
    }
    // ****汤彬修改 移除阴影线
    ...
    if([[[UIDevice currentDevice] systemVersion] floatValue] >= 13.f) {
        [self setupTabImageViewDefaultOffset:self.selectedItem.cyl_tabButton];
    }else{
    ...
    [tabBarSubviews enumerateObjectsUsingBlock:^(UIControl * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj cyl_isTabButton]) {
            ...
            [obj cyl_setTabBarChildViewControllerIndex:idx];
             // *******汤彬修改 调整底部文字往上移3*******
            UIView *view = obj;
            view.frame = CGRectMake(view.frame.origin.x, view.frame.origin.y, view.frame.size.width, view.frame.size.height - 4);
            // *******汤彬修改*******
...
#pragma mark - 注释此方法 为了 适配 iOS13深色模式
