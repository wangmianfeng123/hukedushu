...
- (instancetype)initWithFrame:(CGRect)frame progressMargin:(CGFloat)progressMargin {
    if (self = [super initWithFrame:frame]) {
        self.allowTapped = YES;
        self.progressMargin = progressMargin;
        [self addSubViews];
    }
    return self;
}
...
- (void)layoutSubviews {
    ...
    self.bgProgressView.width       = self.width - self.progressMargin * 2;
...
- (void)addSubViews {
    ...
    self.bgProgressView.frame = CGRectMake(self.progressMargin, 0, 0, kProgressH);
...
- (void)setValue:(float)value {
    ...
    CGFloat buttonX = 0;
    if (self.isBookSlider) {
        buttonX = finishValue;
        if (1 == value) {
            //buttonX  = self.bgProgressView.frame.size.width - self.sliderBtn.width/3;
        }
    }else{
        buttonX = self.bgProgressView.frame.size.width * value;
    }
    self.sliderBtn.x = buttonX;
...
- (void)sliderBtnTouchBegin:(UIButton *)btn {
    ...
    if ([self.delegate respondsToSelector:@selector(sliderTouchBegin:)]) {
        ...
        //self.sliderBtn.transform = CGAffineTransformMakeScale(1.2, 1.2);
    }
}
...
- (void)sliderBtnTouchEnded:(UIButton *)btn {
    ...
    if ([self.delegate respondsToSelector:@selector(sliderTouchEnded:)]) {
        ...
        //self.sliderBtn.transform = CGAffineTransformMakeScale(1, 1);
    }
}
...
- (UIImageView *)bgProgressView {
    if (!_bgProgressView) {
        ...
        _bgProgressView.layer.cornerRadius = 3;
    }
...
- (UIImageView *)bufferProgressView {
    if (!_bufferProgressView) {
        ...
        _bufferProgressView.layer.cornerRadius = 3;
    }
...
- (UIImageView *)sliderProgressView {
    if (!_sliderProgressView) {
        ...
        _sliderProgressView.layer.cornerRadius = 3;
    }
