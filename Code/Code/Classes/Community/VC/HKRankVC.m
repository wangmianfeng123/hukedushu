...
@interface HKRankVC ()<UITableViewDataSource,UITableViewDelegate,HKMomentCellDelegate,HKLiveMomentCellDelegate,HKWorksMomentCellDelegate,TBSrollViewEmptyDelegate,XLPhotoBrowserDelegate,UMpopViewDelegate>
...
- (void)loadListData{
    ...
    [dic setObject:self.orderById forKey:@"orderBy"];
...
- (UITableView*)tableView {
    if (!_tableView) {//138 * Ratio + KNavBarHeight64
        ...
        _tableView.contentInset = UIEdgeInsetsMake(0, 0, 50+ TAR_BAR_XH+KNavBarHeight64 + 40, 0);
...
- (UIView *)tb_emptyView:(UIScrollView *)scrollView network:(TBNetworkStatus)status{
    ...
    if (status == TBNetworkStatusReachableViaWWAN || status== TBNetworkStatusReachableViaWiFi) {
        ...
        UIButton *bulidBtn = [UIButton buttonWithTitle:@"现在去发言" titleColor:COLOR_FF7820 titleFont:@"14" imageName:@""];
        [bulidBtn addTarget:self action:@selector(bulidBtnClick) forControlEvents:UIControlEventTouchUpInside];
        [bulidBtn setContentEdgeInsets:UIEdgeInsetsMake(5, 10, 5, 10)];
        [view addSubview:bulidBtn];
        
        UILabel *titelLB = [UILabel labelWithTitle:CGRectZero title:@"这里欠缺一位像你这样勇于发言的人" titleColor:COLOR_A8ABBE_7B8196 titleFont:@"14" titleAligment:NSTextAlignmentCenter];
        ...
        [titelLB mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(bulidBtn.mas_top).offset(-PADDING_20);
        }];
        
        [bulidBtn addCornerRadius:12.5 addBoderWithColor:COLOR_FF7820];
        return view;
    }
    return nil;
}
