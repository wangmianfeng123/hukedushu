...
//    UIView  *view = isWindow? (UIView*)[UIApplication sharedApplication].delegate.window :[self getCurrentUIVC].view;
    ...
    hud.label.font= [UIFont systemFontOfSize:(IS_IPHONE6PLUS ? 16:15)];
...
+ (void)showTipMessageInWindow:(NSString*)message
    ...
    hud.bezelView.alpha = (alpha>0) ?alpha :0.5f;
    hud.label.numberOfLines = 0;
    hud.contentColor = [UIColor whiteColor];
    hud.label.font = font ? font : HK_FONT_SYSTEM(IS_IPHONE6PLUS ? 16:15);
...
+ (void)showTipMessage:(NSString*)message isWindow:(BOOL)isWindow timer:(int)aTimer {
    ...
    hud.bezelView.color = [COLOR_000000 colorWithAlphaComponent:0.5];
    //hud.bezelView.alpha = 0.5f;
    hud.label.numberOfLines = 0;
    hud.contentColor = COLOR_ffffff;
    [hud hideAnimated:YES afterDelay:aTimer];
}
...
{
    [self showCustomIconInWindow:imageName message:Message];
