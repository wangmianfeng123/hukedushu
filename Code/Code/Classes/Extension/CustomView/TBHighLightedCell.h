...
typedef enum {
    TableViewIndexBack = 0,// 0-cell子控件的最底层 默认
    TableViewIndexFont = 1,// 1-cell子控件的最上层
    TableViewIndexContentViewFont = 2,// 2-contentView子控件的最上层
} TableViewIndex;

@interface TBHighLightedCell : UITableViewCell

@property (nonatomic, strong)UIColor *hightedLigthedColor;// 高亮的背景颜色

@property (nonatomic, assign)CGFloat tb_cornerRadius;// 高亮的圆角

@property (nonatomic, assign)TableViewIndex tb_hightedLigthedIndex;// 高亮View位于哪个一层
