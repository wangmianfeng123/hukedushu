...
- (void)setModelDict:(NSDictionary *)modelDict {
    ...
    // 解析模型数据
    NSDictionary *data = modelDict[@"data"];
    if (self.mine) {
        ...
        [self.imgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(weakSelf.mas_top).offset(3.0);
            make.left.equalTo(weakSelf.mas_right).offset(-30.0);
        ...
        NSDictionary *user = modelDict[@"user"];
        self.avatarView.frame = CGRectMake(10.0, 10.0, 35.0, 35.0);
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:user[@"pic"]] placeholderImage:[UIImage imageNamed:@"plv_img_default_avatar"]];
        ...
        [self.imgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(weakSelf.avatarView.mas_top).offset(25.0);
            make.leading.equalTo(weakSelf.avatarView.mas_trailing).offset(10.0);
            make.size.mas_equalTo(CGSizeMake(20.0, 34.0));
        }];
        _height = 70.0;
    ...
    switch ([data[@"contentType"] integerValue]) {
        case 1: {
