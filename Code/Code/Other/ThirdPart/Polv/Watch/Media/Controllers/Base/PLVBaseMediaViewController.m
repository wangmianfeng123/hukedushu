...
@property (nonatomic, assign) BOOL fullScreen;//标记ipad横屏时，主动触发全屏观看
...
- (CGRect)correctMainRect {
    ...
    if (self.skinView.fullscreen) {
        ...
        if (@available(iOS 11.0, *)) {
            ...
            if (safeFrame.origin.y > 0.0 && IS_IPHONE) {
...
- (void)showDanmuModule{
    if(self.enableDanmuModule){
        ...
        if (self.skinView.openDanmuByUser == 0 || self.skinView.openDanmuByUser == 1) {
            ...
            [self.skinView showDanmuInputBtn:YES]; // 竖屏时无弹幕输入框
...
- (void)changeFrame:(BOOL)fullscreen block:(void (^)(void))block {
    if (fullscreen) {
        self.view.frame = IS_IPAD ?CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT): self.fullRect;
    ...
    NSLog(@"self.fullRect ===== %@",NSStringFromCGRect(self.view.frame));
    NSLog(@"self.fullRect ===== skinView == %@",NSStringFromCGRect(self.skinView.frame));
...
- (void)deviceOrientationDidChange {
    ...
    [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        [weakSelf changeFrame:IS_IPAD ?self.fullScreen : UIDeviceOrientationIsLandscape(orientation) block:^{
...
//横竖屏旋转动画
- (void)manChangedeviceOrientationDidChange:(UIDeviceOrientation)orientation {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        BOOL iPhone = [@"iPhone" isEqualToString:[UIDevice currentDevice].model];
        if (!UIDeviceOrientationIsValidInterfaceOrientation(orientation) || (iPhone && [UIDevice currentDevice].orientation == UIDeviceOrientationPortraitUpsideDown) || [PLVLiveVideoConfig sharedInstance].unableRotate || [PLVLiveVideoConfig sharedInstance].triviaCardUnableRotate) {
            return;
        }
