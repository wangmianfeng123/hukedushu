//
//  ZFHKPlayerPortraitSimilarVideoView.m
//  Code
//
...
#import "ZFHKPlayerPortraitSimilarVideoView.h"


@interface ZFHKPlayerPortraitSimilarVideoView()<UIGestureRecognizerDelegate>
...
@implementation ZFHKPlayerPortraitSimilarVideoView
...
- (void)layoutSubviews {
    ...
    [self.similarLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        if (IS_IPAD) {
            make.top.equalTo(self).offset(SCREEN_HEIGHT * 0.25 - 50);;
            make.left.equalTo(self).offset(SCREEN_WIDTH * 0.3);
        }else{
            make.top.equalTo(self).offset(128/2*Ratio);;
...
- (void)resetBtnClick {
    ...
    if ([self.playerSimilarVideoDelagate respondsToSelector:@selector(ZFHKPlayerPortraitSimilarVideoView:repeatVideo:)]) {
        [self.playerSimilarVideoDelagate ZFHKPlayerPortraitSimilarVideoView:self repeatVideo :self.model];
        [self removeView];
    }
}
...
- (void)viewClick:(id)sender {
    ...
    if ([self.playerSimilarVideoDelagate respondsToSelector:@selector(ZFHKPlayerPortraitSimilarVideoView:similarVideo:)]) {
        [self.playerSimilarVideoDelagate ZFHKPlayerPortraitSimilarVideoView:self similarVideo:self.model];
        [self removeView];
    }
}
