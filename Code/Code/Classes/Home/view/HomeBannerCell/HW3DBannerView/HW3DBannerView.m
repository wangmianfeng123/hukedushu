...
@interface HW3DBannerView ()<XHPageControlDelegate>
/** 页码指示器 */
//@property (nonatomic,strong) HKHomeBannerPageControlView *pageControl;
...
//
//    _pageControl = [[HKHomeBannerPageControlView alloc]initWithFrame:CGRectMake((self.frame.size.width - 200)/2, ZXMainScrollViewHeight - 5 - 8, 200, 5)];
...
//    _pageControl.hidden = !_showPageControl;
    ...
    _pageControl.frame=CGRectMake((self.frame.size.width - 200)/2, ZXMainScrollViewHeight - 25, 200, 25);
    _pageControl.userInteractionEnabled = NO;
    _pageControl.currentPage = 0;
...
- (void)setInfoByCurrentImageIndex:(NSUInteger)currentImageIndex {
    ...
    if([self isHttpString:self.data[currentImageIndex]]){
        [self.centerIV sd_setImageWithURL:[NSURL URLWithString:[HKLoadingImageTool transitionImageUrlString:self.data[currentImageIndex]]] placeholderImage:self.placeHolderImage];
    }else {
    ...
    if([self isHttpString:self.data[leftIndex]]){
        [self.leftIV sd_setImageWithURL:[NSURL URLWithString:[HKLoadingImageTool transitionImageUrlString:self.data[leftIndex]]] placeholderImage:self.placeHolderImage];
    }else {
    ...
    if([self isHttpString:self.data[rightIndex]]){
        [self.rightIV sd_setImageWithURL:[NSURL URLWithString:[HKLoadingImageTool transitionImageUrlString:self.data[rightIndex]]] placeholderImage:self.placeHolderImage];
    }else {
...
-(void)centerTapGes{
    !self.clickCenterBlock? : self.clickCenterBlock(self.currentImageIndex);
}
