...
- (void)viewDidLoad{
    ...
    self.orientationType = ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationPortrait)? LEEScreenOrientationTypeVertical : LEEScreenOrientationTypeHorizontal;
...
@property (nonatomic,strong)UIImageView * _Nullable bgImgView;
@end
...
- (UIImageView *)bgImgView {
    if (!_bgImgView) {
        _bgImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Protocl_icon"]];
...
- (void)configAlert{
    ...
    self.bgImgView.frame = CGRectMake(CGRectGetMinX(self.containerView.frame) + self.containerView.width * 0.5-26, CGRectGetMinY(self.containerView.frame)-29, 52, 58);
    self.bgImgView.hidden = !self.config.showTopImage;
}
