...
+ (NSArray *)musicList {
    ...
    if (!musics) {
        ...
        musics = [VideoModel mj_objectArrayWithKeyValuesArray:data];
        //musics = [NSArray yy_modelArrayWithClass:[VideoModel class] json:data];
    }
    return musics;
}
...
+ (NSInteger)indexFromID:(NSString *)musicID {
    ...
    [[self musicList] enumerateObjectsUsingBlock:^(VideoModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj.audio_id isEqualToString:musicID]) {
            index = idx;
            *stop = YES;
        }
    }];
