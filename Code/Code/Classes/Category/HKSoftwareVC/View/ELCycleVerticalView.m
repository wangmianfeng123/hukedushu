...
- (void)setViewInfo{
    ...
    NSString * txt = _dataSource[_indexNow%(_dataSource.count)];
    _tmpMiddleView.text = txt;
    _tmpMiddleView.attributedText = [self addAtrrbuteWithString:txt];
    
    if(_dataSource.count > 1){
        NSString * txt = _dataSource[(_indexNow+1)%(_dataSource.count)];
        if (_direction == ELCycleVerticalViewScrollDirectionDown) {
            _tmpTopView.text = txt;
            _tmpTopView.attributedText = [self addAtrrbuteWithString:txt];
        } else {
            _tmpBtmView.text = txt;
            _tmpBtmView.attributedText = [self addAtrrbuteWithString:txt];
        }
    }
}


- (NSMutableAttributedString *)addAtrrbuteWithString:(NSString *)txt{
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:txt];
//    if (!self.isCommunity) {
        
//    }else{
//        NSRange range = [txt rangeOfString:@"刚刚解答了提问"];
//        [attrString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"#FF7820"] range:NSMakeRange(range.location, range.length)];
//    }
    if (self.messageArray.count == 0) {
        NSRange range = [txt rangeOfString:@"获得"];
        [attrString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"#FF7820"] range:NSMakeRange(0, range.location)];
        [attrString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"#FF7820"] range:NSMakeRange(range.location+range.length, txt.length-(range.location +range.length)-2)];
    }else{
        for (NSString * message in self.messageArray) {
            if ([txt containsString:message]) {
                NSRange range = [txt rangeOfString:message];
                [attrString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"#FF7820"] range:NSMakeRange(range.location, range.length)];
                break;
            }
        }
