...
@interface HKCommunityAllVC ()<UITableViewDataSource,UITableViewDelegate,HKMomentCellDelegate,HKLiveMomentCellDelegate,HKWorksMomentCellDelegate,XLPhotoBrowserDatasource,XLPhotoBrowserDelegate,XLPhotoBrowserDelegate,UMpopViewDelegate>//
...
@implementation HKCommunityAllVC
...
- (void)loadListData{
    ...
    if (self.typeModel.page_filter.count) {
        for (HKParameterModel * paraModel in self.typeModel.page_filter) {
            [dic setObject:[NSString stringWithFormat:@"%@",paraModel.ParameterValue] forKey:paraModel.ParameterName];
        }
            ...
            if (resultArray.count) {
                for (HKMomentDetailModel* model in resultArray) {
                    ...
                    if (model.dynamic.imageUrl.length) {
                        model.dynamic.imageUrl = [NSString stringWithFormat:@"%@!/format/jpeg/fw/500",model.dynamic.imageUrl];
                    }
                }
...
- (UITableView*)tableView {
    if (!_tableView) {//138 * Ratio + KNavBarHeight64
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(IS_IPAD ? iPadContentMargin : 0.0, 0, IS_IPAD ? iPadContentWidth : SCREEN_WIDTH, SCREEN_HEIGHT-STATUS_BAR_EH) style:UITableViewStylePlain];
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([HKCommunityBannerCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([HKCommunityBannerCell class])];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([HKCarouselCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([HKCarouselCell class])];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (section == 0) {
        if (self.typeModel.hasCarouselMessage) {
            return 2;
        }else{
            return 0;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            HKCarouselCell * cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([HKCarouselCell class])];
            cell.dataArray = self.tabModel.carousel_message;
            WeakSelf
            cell.didItemBlock = ^(HKCarouselModel * _Nonnull model) {
                HKMomentDetailVC * vc = [[HKMomentDetailVC alloc] init];
                vc.topic_id = model.topic_id;
                vc.connect_type = model.connectType;
                [weakSelf.navigationController pushViewController:vc animated:YES];
                
                [MobClick event: community_carousel];
            };
            return cell;
        }else{
            HKCommunityBannerCell * cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([HKCommunityBannerCell class])];
            cell.ad_data = self.tabModel.ad_data;
            WeakSelf
            cell.didImgBlock = ^(HKADdataModel * _Nonnull model) {
                [HKH5PushToNative runtimePush:model.redirect_package.className arr:model.redirect_package.list currectVC:weakSelf];
            };
            return cell;
        }
    }else{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            return self.tabModel.carousel_message.count ? 50 : 0.0;
        }else{
            return self.tabModel.ad_data.count ? 90 : 0.0;;
        }
    }else{
