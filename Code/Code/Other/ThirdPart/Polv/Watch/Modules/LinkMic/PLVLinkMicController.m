...
- (void)viewDidLoad {
    ...
    UIColor *bgColor = [UIColor hkdm_colorWithColorLight:COLOR_27323F dark:COLOR_27323F];
...
- (void)addLiveInfoView {
    ...
    self.nickNameLabel.textColor = COLOR_EFEFF6;
    ...
    self.onLineCountLabel.textColor = COLOR_A8ABBE;
    self.onLineCountLabel.textAlignment = NSTextAlignmentRight;
    ...
    [self.onLineCountLabel setContentHuggingPriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisHorizontal];
    
    [self refreshLiveInfoFlag:NO];
    
    self.wechatBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.wechatBtn.titleLabel setFont:HK_FONT_SYSTEM(13)];
    [self.wechatBtn setTitle:@"联系班主任" forState:UIControlStateNormal];
    [self.wechatBtn setTitle:@"联系班主任" forState:UIControlStateHighlighted];
    [self.wechatBtn setTitleColor:COLOR_A8ABBE_7B8196 forState:UIControlStateNormal];
    [self.wechatBtn setTitleColor:COLOR_A8ABBE_7B8196 forState:UIControlStateHighlighted];
    [self.wechatBtn addTarget:self action:@selector(wechatBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    
    UIImage *image = [UIImage imageNamed:@"ic_live_wechat"];
    [self.wechatBtn setImage:image forState:UIControlStateNormal];
    [self.wechatBtn setImage:image forState:UIControlStateHighlighted];
    [self.wechatBtn layoutButtonWithEdgeInsetsStyle:HKButtonEdgeInsetsStyleLeft imageTitleSpace:5];
    [self.wechatBtn sizeToFit];
    
    [self.liveInfoView addSubview:self.wechatBtn];
}



- (void)wechatBtnClick:(UIButton*)btn {
    if ([self.delegate respondsToSelector:@selector(wechatBtnClickAction:)]) {
        [self.delegate wechatBtnClickAction:self];
    }
}
...
- (void)viewWillLayoutSubviews{
    ...
    self.nickNameLabel.frame = CGRectMake(CGRectGetMaxX(self.avatarImgView.frame) + 10.0,
    ...
    self.onLineCountLabel.frame = CGRectMake(CGRectGetWidth(self.liveInfoView.frame) - 15.0 - onLineCountLabelSize.width,
                                             (CGRectGetHeight(self.liveInfoView.frame)-onLineCountLabelSize.height)/2,
                                             onLineCountLabelSize.width,
                                             onLineCountLabelSize.height);
    ...
    [self.onLineCountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.liveInfoView).offset(-15);
        make.centerY.equalTo(self.avatarImgView);
        make.width.mas_lessThanOrEqualTo(200);
    }];
    
    [self.wechatBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.onLineCountLabel.mas_left).offset(-15);
        make.centerY.equalTo(self.avatarImgView);
    }];
}
