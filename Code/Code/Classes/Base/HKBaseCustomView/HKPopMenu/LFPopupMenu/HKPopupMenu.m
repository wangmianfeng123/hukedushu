...
@implementation HKPopupMenuItem

+ (HKPopupMenuItem *)createWithTitle:(NSString *)title image:(UIImage *)image {
    HKPopupMenuItem *item = [[HKPopupMenuItem alloc] init];
    item.title = title;
    item.image = image;
    return item;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
       ...
       HKPopupMenuDefaultConfig *config = [HKPopupMenuDefaultConfig sharedInstance];
        
        self.rowHeight = config.rowHeight;
        self.arrowH = config.arrowH;
        self.arrowW = config.arrowW;
        self.minWidth = config.minWidth;
    
        self.popupMargin = config.popupMargin;
        self.leftEdgeMargin = config.leftEdgeMargin;
        self.rightEdgeMargin = config.rightEdgeMargin;
        self.textMargin = config.textMargin;
        self.lineMargin = config.lineMargin;
        self.cornerRadius = config.cornerRadius;
        
        self.arrowCornerRadius = config.arrowCornerRadius;
        self.lineColor = config.lineColor;
        self.textFont  = config.textFont;
        self.textColor = config.textColor;
        self.fillColor = config.fillColor;
        
        self.needBorder = config.needBorder;
        self.shadowColor = config.shadowColor;
        self.shadowOfMenu = config.shadowOfMenu;
...
- (void)configWithItems:(NSArray<HKPopupMenuItem *>*)items action:(void(^)(NSInteger index))action {
    ...
    for (NSInteger i = 0; i < self.menuItems.count; i++) {
        HKPopupMenuItem *item = self.menuItems[i];
...
- (void)dismissPopMenu {
    HKPopUpMenuAnimationStyle style = self.animationStyle;
    if (style == HKPopUpMenuAnimationStyleFade) {
        self.alpha = 1;
    ...
    } else if (style == HKPopUpMenuAnimationStyleNone) {
        [self dismiss];
    }
}
...
- (void)addToWindow {
    ...
    [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismissPopMenu)];
...
- (UIView *)maskView {
    if (!_maskView) {
        ...
        _maskView.backgroundColor = [UIColor clearColor];//[[UIColor blackColor] colorWithAlphaComponent:0.2];
    }
    return _maskView;
}
