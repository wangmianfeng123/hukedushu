...
- (UIImage *)placeholderImage
{
    if (!_placeholderImage) {
        _placeholderImage = imageName(HK_Placeholder);
        //[UIImage xl_imageWithColor:[UIColor grayColor] size:CGSizeMake(100, 100)];
    }
...
- (UIWindow *)photoBrowserWindow
{
    if (!_photoBrowserWindow) {
        ...
        UIViewController *tempVC = [self photoBrowserVC];
...
- (XLPhotoBrowserVC*)photoBrowserVC {
    
    return [XLPhotoBrowserVC new];
}
...
- (void)setCustomPageControlDotImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if(@available(iOS 14.0, *)) {
            ...
            pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#FFDC21"];
        }else{
            if (isCurrentPageDot) {
...
- (void)updateFrames
{
    ...
    self.indexLabel.xl_centerY = 35 + KNavBarHeight64;
...
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex
{
    ...
    // 解决特殊字符乱码问题
    NSMutableArray *utf8StringArray = [images mutableCopy];
    for (int i = 0; i < utf8StringArray.count; i++) {
        NSString *image = utf8StringArray[i];
        if ([image isKindOfClass:[NSString class]]) {
            NSString *stringTemp = [image stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            [utf8StringArray replaceObjectAtIndex:i withObject:stringTemp];
        }
    }
    images = utf8StringArray;
