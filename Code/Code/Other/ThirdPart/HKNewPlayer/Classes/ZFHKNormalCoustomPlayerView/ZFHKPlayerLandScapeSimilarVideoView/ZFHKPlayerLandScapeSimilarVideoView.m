//
//  ZFHKPlayerLandScapeSimilarVideoView.m
//  Code
//
...
#import "ZFHKPlayerLandScapeSimilarVideoView.h"
#import "ZFHKPlayerLandScapeSimilarVideoCollectionView.h"



@interface ZFHKPlayerLandScapeSimilarVideoView() <ZFHKPlayerLandScapeSimilarVideoCollectionViewDelagate>

@property (nonatomic,strong) UILabel *tagLabel;

@property (nonatomic,strong) UIButton *resetBtn;

@property (nonatomic,strong) ZFHKPlayerLandScapeSimilarVideoCollectionView *playerCollectionView;
...
@implementation ZFHKPlayerLandScapeSimilarVideoView
...
- (void)layoutSubviews {
    ...
    [self.tagLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self).offset(-245/2 - 50);
...
- (ZFHKPlayerLandScapeSimilarVideoCollectionView*)playerCollectionView {
    
    if (!_playerCollectionView) {
        _playerCollectionView = [[ZFHKPlayerLandScapeSimilarVideoCollectionView alloc]initWithFrame:CGRectZero collectionViewLayout:[self layout]];
...
- (void)zfHKPlayerLandScapeSimilarVideoCollectionView:(UICollectionView*)collectionView videoModel:(VideoModel*)model {
    if (self.delegate && [self.delegate respondsToSelector:@selector(zfHKPlayerLandScapeSimilarVideoView:videoModel:)]) {
        [self.delegate zfHKPlayerLandScapeSimilarVideoView:collectionView videoModel:model];
        [self removeView];
    }
}
...
- (void)repeatVidoClick:(UIButton*)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(zfHKPlayerLandScapeSimilarVideoView:repeatBtnClick:)]) {
        [self.delegate zfHKPlayerLandScapeSimilarVideoView:self repeatBtnClick:nil];
        [self removeView];
    }
}
