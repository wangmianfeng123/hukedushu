...
- (void)layoutSubviews {
    ...
    self.needleView.centerX = SCREEN_WIDTH * 0.5 + 25;
    
    self.needleView.y       = -25;
...
- (void)setCurrentIndex:(NSInteger)currentIndex needChange:(BOOL)needChange {
    if (currentIndex >= 0) {
        ...
        VideoModel *leftM   = self.musics[leftIndex];
        VideoModel *centerM = self.musics[currentIndex];
        VideoModel *rightM  = self.musics[rightIndex];
        
        // 设置图片
        self.centerDiskView.imgurl = centerM.cover_url;
        ...
        if (needChange) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                self.leftDiskView.imgurl   = leftM.cover_url;
                self.rightDiskView.imgurl  = rightM.cover_url;
...
//        NSLog(@"滑动中，当前索引%zd", self.currentIndex);
        ...
        VideoModel *model = self.musics[idx];
...
- (void)scrollViewDidEnd:(UIScrollView *)scrollView {
    ...
    VideoModel *model = self.musics[self.currentIndex];
...
- (UIView *)sepLineView {
    if (!_sepLineView) {
        ...
        _sepLineView.backgroundColor = [UIColor clearColor];
    }
    return _sepLineView;
}
...
- (UIView *)diskBgView {
    if (!_diskBgView) {
        ...
        _diskBgView.backgroundColor = [COLOR_000000 colorWithAlphaComponent:0.2];
        CGFloat leftMagin = 62*2;
        _diskBgView.layer.cornerRadius = (SCREEN_WIDTH - leftMagin) * 0.5;
        _diskBgView.layer.masksToBounds = YES;
    }
    return _diskBgView;
}
...
- (UIScrollView *)diskScrollView {
    if (!_diskScrollView) {
        ...
        _diskScrollView.userInteractionEnabled = NO;
    }
...
- (GKWYDiskView *)centerDiskView {
    if (!_centerDiskView) {
        ...
        _centerDiskView.backgroundColor = [UIColor clearColor];
    }
