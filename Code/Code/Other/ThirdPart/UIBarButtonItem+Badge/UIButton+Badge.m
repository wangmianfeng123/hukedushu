...
NSString const *UIButton_badgeTopPaddingKey = @"UIButton_badgeTopPaddingKey";
...
- (void)updateBadgeFrame
{
    ...
    CGFloat badgeTopPadding = 2*self.badgeTopPadding;
    // Using const we make sure the badge doesn't get too smal
    minWidth = (minWidth < minHeight) ? minHeight : expectedLabelSize.width;
    self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, minWidth + padding+8, minHeight + badgeTopPadding);
    self.badge.layer.cornerRadius = (minHeight + badgeTopPadding) / 2;
...
-(CGFloat) badgeTopPadding {
    NSNumber *number = objc_getAssociatedObject(self, &UIButton_badgeTopPaddingKey);
    return number.floatValue;
}


- (void) setBadgeTopPadding:(CGFloat)badgeTopPadding {
    NSNumber *number = [NSNumber numberWithDouble:badgeTopPadding];
    objc_setAssociatedObject(self, &UIButton_badgeTopPaddingKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.badge) {
        [self updateBadgeFrame];
    }
}
