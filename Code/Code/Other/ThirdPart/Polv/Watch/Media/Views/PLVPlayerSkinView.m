...
- (IBAction)danmuBtnAction:(id)sender {
    ...
    if (orientation == UIDeviceOrientationFaceUp || orientation == UIDeviceOrientationFaceDown || orientation == UIDeviceOrientationUnknown || orientation == UIDeviceOrientationPortraitUpsideDown) {
        isNoNeedOrientation = YES;
    }
    if (isNoNeedOrientation) {
        // 不确定的方向
        [self showDanmuInputBtn:(self.danmuBtn.selected && self.fullscreen)];
    }else{
...
- (IBAction)backBtnAction:(id)sender {
    if (self.fullscreen) {
        if (IS_IPAD) {
            if (self.changeDeviceOrientationBlock) {
                self.changeDeviceOrientationBlock(UIDeviceOrientationPortrait);
            }
        }else{
...
- (IBAction)zoomScreenBtnAction:(id)sender {
    if (IS_IPAD) {
        if (self.changeDeviceOrientationBlock) {
            ...
            if (orientations == UIInterfaceOrientationLandscapeRight){
                orientation = UIDeviceOrientationLandscapeRight;
            }else{
                orientation = UIDeviceOrientationLandscapeLeft ;
            }
            self.changeDeviceOrientationBlock(orientation);
        }
    }else{
        [PCCUtils changeDeviceOrientation:UIDeviceOrientationLandscapeLeft];
        if(@available(iOS 16.0 , *)){
            self.changeDeviceOrientationBlock(UIDeviceOrientationLandscapeLeft);
        }
    }
}
...
- (void)layout {
    ...
    //showDanmuMargin = UIEdgeInsetsMake(-1.0, lrPadding + btnPadding * 2 + 35.0, 30.0, lrPadding + btnPadding * 2 + 35.0);
